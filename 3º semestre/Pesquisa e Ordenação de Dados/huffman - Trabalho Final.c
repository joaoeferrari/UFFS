...
// novo nó
struct Node* criaNo(char data, unsigned int frequencia) {
...
void swapNo(struct Node** a, struct Node** b) {
    struct Node* aux = *a;
    *a = *b;
    *b = aux;
}
    ...
    for (i = 0; i < size - 1; i++) {
        for (j = 0; j < size - i - 1; j++) {
            if (floresta[j]->frequencia > floresta[j + 1]->frequencia || (floresta[j]->frequencia == floresta[j+1]->frequencia && floresta[j]->data > floresta[j+1]->data)) {
                swapNo(&floresta[j], &floresta[j + 1]);
            }
        }
    }
}
...
struct Node* mergeArvores(struct Node* left, struct Node* right) {
    struct Node* novo = criaNo('\0', left->frequencia + right->frequencia);
    novo->left = left;
    novo->right = right;
    return novo;
}
...
struct Node* criacaoArvore(char* caracter) {
    ...
    for (i = 0; i < 256; i++) {
        if (frequencies[i] > 0) {
            floresta[tamanhoFloresta++] = criaNo((char)i, frequencies[i]);
        }
    }
